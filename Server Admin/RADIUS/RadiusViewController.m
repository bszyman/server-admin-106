...
#import "RadiusOverviewViewController.h"
#import "RadiusLogsViewController.h"
#import "RadiusBaseStationsViewController.h"
                         ...
                         bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        [self.tabs setNotificationName:@"RadiusNavUpdate"];
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(changeModuleSubnavigation:)
                                                     name:@"RadiusNavUpdate"
                                                   object:self.tabs];
        
        NSViewController *vc = [[RadiusOverviewViewController alloc] initWithNibName:@"RadiusOverviewViewController"
...
- (void)changeModuleSubnavigation:(NSNotification *)n
{
    ...
    if ([tabTitle isEqualToString:@"Logs"]) {
        if ([self.panels objectForKey:@"logs"]) {
            ...
            NSViewController *vc = [[RadiusLogsViewController alloc] initWithNibName:@"RadiusLogsViewController"
                                                                                       bundle:[NSBundle mainBundle]];
            [self.panels setObject:vc forKey:@"logs"];
            [self.panelView addSubview:vc.view];
        }
    } else if ([tabTitle isEqualToString:@"Base Stations"]) {
        if ([self.panels objectForKey:@"basestations"]) {
            NSViewController *vc = [self.panels objectForKey:@"basestations"];
            [self.panelView addSubview:vc.view];
        } else {
            NSViewController *vc = [[RadiusBaseStationsViewController alloc] initWithNibName:@"RadiusBaseStationsViewController"
                                                                                       bundle:[NSBundle mainBundle]];
            [self.panels setObject:vc forKey:@"basestations"];
            [self.panelView addSubview:vc.view];
        }
        ...
        if ([self.panels objectForKey:@"settings"]) {
            ...
            NSViewController *vc = [[RadiusSettingsViewController alloc] initWithNibName:@"RadiusSettingsViewController"
                                                                                       bundle:[NSBundle mainBundle]];
